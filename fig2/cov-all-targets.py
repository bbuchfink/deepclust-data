...
def finalize_query(
    ...
    coverage = cov / qlen if qlen > 0 else 0.0
    out_fh.write(f"{q}\t{cov}\t{qlen}\t{coverage:.{decimals}f}\n")
        ...
        with open_maybe_gzip(args.alignments, "rt") as fh:
            for ln, raw in enumerate(fh, 1):
                ...
                if current_q is None:
                    current_q = q
                if q != current_q:
                    finalize_query(current_q, ivs, qlen_map, out_fh, args.decimals, args.debug)
                    ivs.clear()
                    current_q = q
